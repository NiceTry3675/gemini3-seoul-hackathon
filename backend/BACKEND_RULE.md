...
<본문> (변경 이유, 주요 변경사항, 영향 범위를 상세히 기술)
```
...
- **왜(Why)** 변경했는지 반드시 기술
- **무엇(What)**이 변경되었는지 주요 항목 나열
- 영향받는 도메인, 엔드포인트, 스키마 변경사항 명시
- 하위 호환성 깨짐이 있으면 `[Breaking]` 표기

### 8.5 예시

```
기능: 비디오 생성 도메인 추가 (Veo 3.1)

Veo 3.1 API를 wrapping하는 video_generation 도메인 구현.
- POST /api/video/generate 엔드포인트 추가
- Scene Extension, Reference Images, First/Last Frame 지원
- 모든 Veo 기능은 선택적 필드로 하위 호환 유지
- config.py에 VIDEO_MODEL 설정 추가
```
