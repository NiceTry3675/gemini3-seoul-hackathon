...
class TestGeminiTextServiceGenerate:
    def test_generate_returns_text(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("Hello!")
        service = _make_service(mock_genai_client)
        request = TextGenerationRequest(prompt="Say hello")
        result = service.generate(request)
        assert result.text == "Hello!"

    def test_generate_includes_usage_metadata(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response(
            "Hi", prompt_tokens=8, candidates_tokens=3
        )
        service = _make_service(mock_genai_client)
        result = service.generate(TextGenerationRequest(prompt="Greet"))
    ...
    def test_generate_handles_none_text(self, mock_genai_client):
        response = MagicMock()
        response.text = None
        response.usage_metadata = None
        mock_genai_client.models.generate_content.return_value = response
        service = _make_service(mock_genai_client)
        result = service.generate(TextGenerationRequest(prompt="test"))
        assert result.text == ""
        assert result.usage_metadata is None

    def test_generate_passes_temperature_and_max_tokens(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("ok")
        service = _make_service(mock_genai_client)
    ...
    def test_generate_raises_quota_exceeded_on_429(self, mock_genai_client):
        ...
        with pytest.raises(QuotaExceededError):
            service.generate(TextGenerationRequest(prompt="test"))
...
class TestGeminiTextServiceGenerateStructured:
    ...
    def test_returns_parsed_dict(self, mock_genai_client):
        expected = {"name": "Alice", "age": 30}
        mock_genai_client.models.generate_content.return_value = self._make_json_response(expected)
        service = _make_service(mock_genai_client)
    ...
    def test_strips_markdown_json_fence(self, mock_genai_client):
        response = MagicMock()
        response.text = '```json\n{"key": "value"}\n```'
        response.usage_metadata = None
        mock_genai_client.models.generate_content.return_value = response
    ...
    def test_strips_plain_code_fence(self, mock_genai_client):
        response = MagicMock()
        response.text = '```\n{"key": "value"}\n```'
        response.usage_metadata = None
        mock_genai_client.models.generate_content.return_value = response
    ...
    def test_raises_gemini_api_error_on_invalid_json(self, mock_genai_client):
        response = MagicMock()
    ...
    def test_raises_quota_exceeded(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = Exception("429 quota")
        service = _make_service(mock_genai_client)
        with pytest.raises(QuotaExceededError):
            service.generate_structured(
                StructuredGenerationRequest(prompt="test", response_schema={})
    ...
    def test_raises_safety_block(self, mock_genai_client):
    ...
    def test_empty_response_returns_empty_dict(self, mock_genai_client):
        response = MagicMock()
        response.text = None
        response.usage_metadata = None
        mock_genai_client.models.generate_content.return_value = response
...
class TestTextGenerationRouter:
    def test_generate_returns_200(self, test_client, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("Generated text")
    ...
    def test_structured_returns_200(self, test_client, mock_genai_client):
        ...
        mock_genai_client.models.generate_content.return_value = response_mock
        response = test_client.post(
            "/api/text/structured",
            json={
...
class TestGeminiTextServiceMultimodal:
    def test_no_parts_passes_plain_string(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("ok")
        service = _make_service(mock_genai_client)
    ...
    def test_single_part_builds_part_list(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("ok")
        service = _make_service(mock_genai_client)
    ...
    def test_multiple_parts_builds_all(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("ok")
        service = _make_service(mock_genai_client)
        img_data = base64.b64encode(b"fake-image").decode()
    ...
    def test_parts_default_empty_is_backward_compatible(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("ok")
        service = _make_service(mock_genai_client)
    ...
    def test_structured_with_parts(self, mock_genai_client):
        response = MagicMock()
...
class TestTextMultimodalRouter:
    def test_generate_with_parts_returns_200(self, test_client, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("Described!")
    ...
    def test_generate_with_multiple_parts_returns_200(self, test_client, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = _make_text_response("Analysis complete")
        img_data = base64.b64encode(b"fake-img").decode()
        audio_data = base64.b64encode(b"fake-audio").decode()
        response = test_client.post(
            "/api/text/generate",
