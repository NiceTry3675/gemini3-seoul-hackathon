...
class TestSceneParserServiceHappyPath:
    def test_parse_returns_scene_breakdown(self, mock_genai_client):
        ...
        result = service.parse(NovelInput(
    ...
    def test_parse_populates_scene_fields_correctly(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = mock_structured_response(
            _valid_scene_breakdown_json()
        )
        service = _make_service(mock_genai_client)
        result = service.parse(NovelInput(manuscript="test", genre="drama", tone="dark"))
        scene = result.scenes[0]
        assert scene.scene_number == 1
        assert scene.characters == ["Alice"]
        assert scene.key_dialogue == ["Hello?"]

    def test_parse_calls_generate_content_with_json_mime_type(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = mock_structured_response(
            _valid_scene_breakdown_json()
        )
        service = _make_service(mock_genai_client)
        service.parse(NovelInput(manuscript="test", genre="thriller", tone="dark"))
        call_kwargs = mock_genai_client.models.generate_content.call_args
        config = call_kwargs.kwargs.get("config") or call_kwargs[1].get("config")
        assert config.response_mime_type == "application/json"

    def test_parse_uses_system_instruction_from_prompt_manager(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = mock_structured_response(
            _valid_scene_breakdown_json()
        )
        service = _make_service(mock_genai_client)
        service.parse(NovelInput(manuscript="test", genre="romance", tone="warm"))
        call_kwargs = mock_genai_client.models.generate_content.call_args
        config = call_kwargs.kwargs.get("config") or call_kwargs[1].get("config")
...
class TestSceneParserManuscriptBoundary:
    def test_parse_accepts_max_length_manuscript(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = mock_structured_response(
            _valid_scene_breakdown_json()
        )
        service = _make_service(mock_genai_client)
        manuscript = "a" * 200_000
        result = service.parse(NovelInput(manuscript=manuscript, genre="epic", tone="grand"))
        assert isinstance(result, SceneBreakdown)
    ...
    def test_parse_handles_short_single_sentence_manuscript(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = mock_structured_response(
            _valid_scene_breakdown_json()
        )
        service = _make_service(mock_genai_client)
        result = service.parse(NovelInput(manuscript="A.", genre="drama", tone="quiet"))
        assert isinstance(result, SceneBreakdown)
...
class TestSceneParserMarkdownFences:
    def test_parse_strips_json_markdown_fence(self, mock_genai_client):
        fenced = f"```json\n{_valid_scene_breakdown_json()}\n```"
        mock_genai_client.models.generate_content.return_value = mock_structured_response(fenced)
        service = _make_service(mock_genai_client)
        result = service.parse(NovelInput(manuscript="test", genre="sci-fi", tone="cold"))
        assert isinstance(result, SceneBreakdown)
        assert len(result.scenes) == 2

    def test_parse_strips_plain_code_fence(self, mock_genai_client):
        fenced = f"```\n{_valid_scene_breakdown_json()}\n```"
        mock_genai_client.models.generate_content.return_value = mock_structured_response(fenced)
        service = _make_service(mock_genai_client)
        result = service.parse(NovelInput(manuscript="test", genre="fantasy", tone="dark"))
        assert isinstance(result, SceneBreakdown)
...
class TestSceneParserErrorHandling:
    def test_parse_raises_gemini_api_error_on_invalid_json(self, mock_genai_client):
        mock_genai_client.models.generate_content.return_value = mock_structured_response(
            "not valid json {{{}"
        )
        service = _make_service(mock_genai_client)
        with pytest.raises(GeminiAPIError):
            service.parse(NovelInput(manuscript="test", genre="horror", tone="dark"))

    def test_parse_raises_quota_exceeded_on_429(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = Exception("429 quota exceeded")
        service = _make_service(mock_genai_client)
        with pytest.raises(QuotaExceededError):
            service.parse(NovelInput(manuscript="test", genre="action", tone="fast"))

    def test_parse_raises_quota_exceeded_on_resource_exhausted(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = Exception("RESOURCE_EXHAUSTED limit")
        service = _make_service(mock_genai_client)
        with pytest.raises(QuotaExceededError):
            service.parse(NovelInput(manuscript="test", genre="action", tone="fast"))

    def test_parse_raises_safety_block_on_safety_error(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = Exception("safety filter triggered")
        service = _make_service(mock_genai_client)
        with pytest.raises(SafetyBlockError):
            service.parse(NovelInput(manuscript="test", genre="horror", tone="violent"))

    def test_parse_raises_safety_block_on_block_error(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = Exception("content blocked by policy")
        service = _make_service(mock_genai_client)
        with pytest.raises(SafetyBlockError):
            service.parse(NovelInput(manuscript="test", genre="horror", tone="violent"))

    def test_parse_raises_gemini_api_error_on_generic_exception(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = Exception("network timeout")
        service = _make_service(mock_genai_client)
        with pytest.raises(GeminiAPIError):
            service.parse(NovelInput(manuscript="test", genre="drama", tone="slow"))

    def test_parse_re_raises_quota_exceeded_without_wrapping(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = QuotaExceededError()
        service = _make_service(mock_genai_client)
        with pytest.raises(QuotaExceededError):
            service.parse(NovelInput(manuscript="test", genre="drama", tone="slow"))

    def test_parse_re_raises_safety_block_without_wrapping(self, mock_genai_client):
        mock_genai_client.models.generate_content.side_effect = SafetyBlockError()
        service = _make_service(mock_genai_client)
        with pytest.raises(SafetyBlockError):
            service.parse(NovelInput(manuscript="test", genre="drama", tone="slow"))

    def test_parse_handles_response_text_none(self, mock_genai_client):
