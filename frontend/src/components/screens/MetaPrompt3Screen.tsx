        ...
        <section className="mx-auto mt-10 w-full max-w-[980px] space-y-8">
          <div className="space-y-2">
            ...
            <p className="text-lg text-slate-400">
              smoke 스크립트처럼 프롬프트를 확인/수정한 뒤, 선택한 레퍼런스를 포함해 최종 이미지를 생성합니다.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-700 bg-[#111827] p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="rounded-md border border-ts-border px-3 py-1.5 text-xs font-semibold text-slate-200 hover:bg-slate-800"
              >
                Reset Prompts
              </button>
            </div>
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-slate-700 bg-slate-900">
              {selectedReferenceImage ? (
                <img
                  src={`data:${selectedReferenceImage.mimeType};base64,${selectedReferenceImage.imageBase64}`}
          ...
          <div className="rounded-2xl border border-slate-700 bg-[#111827] p-5">
            ...
            <pre className="teaser-scrollbar mt-2 max-h-40 overflow-auto rounded-lg border border-slate-700 bg-[#0f172a] p-3 text-xs leading-relaxed text-slate-200">
              {anchorPrompt || 'No anchor prompt'}
            </pre>
          </div>
            ...
            {cutPrompts.map((item) => (
              <div key={item.cutNumber} className="space-y-2 rounded-lg border border-slate-700 bg-[#0f172a] p-3">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">
                  Cut {String(item.cutNumber).padStart(2, '0')}
                </p>
                <textarea
                  className="teaser-scrollbar h-24 w-full resize-y rounded border border-slate-700 bg-[#111827] px-3 py-2 text-sm text-slate-100 outline-none focus:border-ts-primary"
                  value={item.prompt}
                  onChange={(event) => onCutPromptChange(item.cutNumber, event.target.value)}
                />
              </div>
            ))}
          </div>
          ...
          {!canExport ? (
            <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-4 text-sm text-red-200">
              Visuals 단계에서 레퍼런스 이미지를 선택해야 Export를 시작할 수 있습니다.
            </div>
          ) : null}
        </section>
      </main>
