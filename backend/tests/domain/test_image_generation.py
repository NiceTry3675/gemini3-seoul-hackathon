...
class TestGeminiImageServiceGenerate:
    async def test_generate_returns_base64_image(self, mock_genai_client):
        raw = b"fake png bytes"
        mock_genai_client.aio.models.generate_content.return_value = _make_image_response(raw)
        service = _make_service(mock_genai_client)
        result = await service.generate(ImageGenerationRequest(prompt="A cat"))
        expected_b64 = base64.b64encode(raw).decode("utf-8")
        assert result.image_base64 == expected_b64

    async def test_generate_returns_correct_mime_type(self, mock_genai_client):
        raw = b"jpeg bytes"
        mock_genai_client.aio.models.generate_content.return_value = _make_image_response(
            raw, mime_type="image/jpeg"
        )
        service = _make_service(mock_genai_client)
        result = await service.generate(ImageGenerationRequest(prompt="A dog"))
        assert result.mime_type == "image/jpeg"

    async def test_generate_defaults_to_png_when_no_mime(self, mock_genai_client):
        inline_data = MagicMock()
        ...
        service = _make_service(mock_genai_client)
        result = await service.generate(ImageGenerationRequest(prompt="test"))
        assert result.mime_type == "image/png"

    async def test_generate_raises_when_no_image_in_response(self, mock_genai_client):
        mock_genai_client.aio.models.generate_content.return_value = _make_no_image_response()
        service = _make_service(mock_genai_client)
        with pytest.raises(GeminiAPIError, match="No image generated"):
            await service.generate(ImageGenerationRequest(prompt="A landscape"))
    ...
    async def test_generate_raises_quota_exceeded_on_429(self, mock_genai_client):
        ...
        with pytest.raises(QuotaExceededError):
            await service.generate(ImageGenerationRequest(prompt="test"))
    ...
    async def test_generate_raises_gemini_api_error_on_generic_exception(self, mock_genai_client):
        mock_genai_client.aio.models.generate_content.side_effect = Exception("connection error")
        service = _make_service(mock_genai_client)
        with pytest.raises(GeminiAPIError):
            await service.generate(ImageGenerationRequest(prompt="test"))

    async def test_generate_re_raises_gemini_api_error(self, mock_genai_client):
        mock_genai_client.aio.models.generate_content.side_effect = GeminiAPIError("custom error")
        service = _make_service(mock_genai_client)
        with pytest.raises(GeminiAPIError):
            await service.generate(ImageGenerationRequest(prompt="test"))

    async def test_generate_with_reference_images_builds_multipart_contents(self, mock_genai_client):
    ...
    async def test_generate_without_reference_images_uses_string_prompt(self, mock_genai_client):
        raw = b"generated image"
        mock_genai_client.aio.models.generate_content.return_value = _make_image_response(raw)
        service = _make_service(mock_genai_client)
        await service.generate(ImageGenerationRequest(prompt="A cat"))
        call_args = mock_genai_client.aio.models.generate_content.call_args
        contents = call_args.kwargs.get("contents") or call_args[1].get("contents")
        assert isinstance(contents, str)

    async def test_generate_skips_text_parts_to_find_image(self, mock_genai_client):
...
class TestImageGenerationRouter:
    ...
    def test_generate_generic_error_returns_502(self, test_client, mock_genai_client):
        mock_genai_client.aio.models.generate_content.side_effect = Exception("unknown error")
        response = test_client.post("/api/image/generate", json={"prompt": "test"})
        assert response.status_code == 502
