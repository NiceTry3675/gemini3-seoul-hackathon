...
class PipelineRepository:
    ...
    async def create_run(self, request: ContiRequest) -> str:
        ...
        await self._db.execute(
            "INSERT INTO pipeline_runs (id, manuscript, genre, tone, output_language, output_mode, status, started_at) "
            "VALUES (?, ?, ?, ?, ?, ?, 'running', ?)",
            (run_id, request.manuscript, request.genre, request.tone, request.output_language, request.output_mode, now),
        )
    ...
    async def save_result(self, run_id: str, result: ContiResult) -> None:
        ...
        for cut in result.cuts:
            await self._db.execute(
                "INSERT INTO pipeline_cuts "
                "(run_id, cut_number, image_base64, mime_type, video_base64, video_mime_type, dialogue_json, narration, description) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    run_id,
                    cut.cut_number,
    ...
    async def list_runs(self) -> list[dict]:
        cursor = await self._db.execute(
            "SELECT r.id, r.manuscript, r.genre, r.tone, r.output_mode, r.status, r.started_at, r.finished_at, "
        ...
        result["cuts"] = [
            {
                ...
                "video_base64": c["video_base64"],
                "video_mime_type": c["video_mime_type"],
