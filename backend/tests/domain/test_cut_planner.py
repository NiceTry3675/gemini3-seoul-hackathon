...
class TestCutPlannerServiceHappyPath:
    def test_plan_returns_cut_plan(
        ...
        mock_genai_client.models.generate_content.return_value = (
            mock_structured_response(_valid_cut_plan_json())
        )
    ...
    def test_plan_returns_exactly_9_cuts(
        self,
        mock_genai_client,
        sample_novel_input,
        sample_scene_breakdown,
    ...
    def test_plan_cut_numbers_are_1_through_9(
        self,
        mock_genai_client,
        sample_novel_input,
        sample_scene_breakdown,
...
class TestCutPlannerErrorHandling:
    def test_plan_raises_gemini_api_error_on_invalid_json(
        ...
        mock_genai_client.models.generate_content.return_value = (
            mock_structured_response("not valid json {{{")
        )
        service = _make_service(mock_genai_client)
        req = CutPlanRequest(
            novel_input=sample_novel_input,
...
class TestCutPlannerRouter:
    def _valid_payload(
        self, sample_novel_input, sample_scene_breakdown, sample_character_sheet
    ):
        return {
