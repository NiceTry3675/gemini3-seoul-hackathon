...
def mock_prompt_manager():
    mock_pm = MagicMock()
    mock_pm.get_system_instruction.return_value = "You are a character generator."
    with patch("app.domain.character_gen.service.get_prompt_manager", return_value=mock_pm):
        yield mock_pm
...
class TestCharacterGenServiceHappyPath:
    @pytest.mark.asyncio
    async def test_generate_returns_character_gen_response(
        ...
        mock_genai_client.aio.models.generate_content.side_effect = [sheet_resp, img_resp, img_resp]
    ...
    @pytest.mark.asyncio
    async def test_generate_includes_reference_images_for_each_character(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_single_character_has_one_reference_image(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
        sheet_resp = mock_structured_response(_character_sheet_json(["Alice"]))
        img_resp = _make_image_response()
        mock_genai_client.aio.models.generate_content.side_effect = [sheet_resp, img_resp]
    ...
    @pytest.mark.asyncio
    async def test_generate_four_characters_all_images_succeed(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_reference_images_contain_base64_strings(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
        import base64
        raw = b"image-data"
...
class TestCharacterGenImageFailures:
    @pytest.mark.asyncio
    async def test_reference_image_failure_results_in_empty_dict(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
        sheet_resp = mock_structured_response(_character_sheet_json(["Alice"]))
        mock_genai_client.aio.models.generate_content.side_effect = [
    ...
    @pytest.mark.asyncio
    async def test_all_reference_images_fail_returns_empty_reference_images(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
        sheet_resp = mock_structured_response(_character_sheet_json(["Alice", "Bob"]))
        mock_genai_client.aio.models.generate_content.side_effect = [
    ...
    @pytest.mark.asyncio
    async def test_partial_image_failures_only_include_successful_images(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
        sheet_resp = mock_structured_response(_character_sheet_json(["Alice", "Bob"]))
        img_resp = _make_image_response()
    ...
    @pytest.mark.asyncio
    async def test_image_response_with_no_inline_data_returns_none_image(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_quota_error_in_image_gen_is_silently_caught(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
...
class TestCharacterGenSheetErrors:
    @pytest.mark.asyncio
    async def test_invalid_json_in_sheet_response_raises_gemini_api_error(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_quota_exceeded_in_sheet_gen_propagates(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_safety_block_in_sheet_gen_propagates(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_generic_exception_in_sheet_gen_raises_gemini_api_error(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
    ...
    @pytest.mark.asyncio
    async def test_markdown_fenced_sheet_response_is_parsed(
        self, mock_genai_client, sample_novel_input, sample_scene_breakdown
    ):
...
class TestCharacterGenRouter:
    ...
    def test_character_gen_returns_502_on_api_error(
        ...
        mock_genai_client.aio.models.generate_content.side_effect = Exception("network error")
        response = test_client.post(
            "/api/pipeline/character-gen",
            json=self._valid_payload(sample_novel_input, sample_scene_breakdown),
        )
